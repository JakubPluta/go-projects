    ...
    <main class="mx-10 min-h-[80vh]">
        <div class="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-3 my-6">
            <div v-for="peer in allPeers" :key="peer.id" class="relative">
                <video autoplay :muted="peer.isLocal" playsinline class="h-full w-full object-cover" :ref="
                  (el) => {
            ...
            left-0
          ">
                    <span class="inline-block w-6" v-show="
                    ...
                    ">
                        <svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <path fill="#FFF" d="m9 17.32c0-.11 0-.21 0-.32v-3h-2v3a9 9 0 0 0 .25 2.09z" />
                ...
                <p class="text-white text-center absolute top-1/2 right-0 left-0" v-show="
        ...
        <div class="mx-auto mt-10 flex items-center justify-center" v-if="allPeers.length">
            ...
            <button class="bg-indigo-400 text-white rounded-md p-3 block mx-5" @click="toggleVideo">
